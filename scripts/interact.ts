...
async function main() {
    ...
    // Starts on coston2, sends funds using lc to coston and deploy them to uniswap liquidity pool
    // await startLiquidityFarming(deployerOrigin)
    // return

    // Remove liquidity directly (no layer cake priming)
    // The drawback is that you have to start on coston
    await removeLiquidityDirect()


    // Remove liquidity using layer cake
